...
from flux2.sampling import (
    denoise, get_schedule, encode_image_refs,
    batched_prc_img, batched_prc_txt, default_prep
)
...
def handler(job):
    ...
    # Collect input images (person + clothing)
    input_images = []
    ...
    with torch.inference_mode():
        ...
        # Prepare output image noise
        h_lat = height // 16
        w_lat = width // 16
        img_noise = torch.randn(1, h_lat * w_lat, 64, device=device, dtype=torch.bfloat16, generator=rng)
        img_ids_tensor = torch.zeros(1, h_lat * w_lat, 4, device=device, dtype=torch.int32)
        for i in range(h_lat):
            for j in range(w_lat):
                img_ids_tensor[0, i * w_lat + j, 1] = i
                img_ids_tensor[0, i * w_lat + j, 2] = j
